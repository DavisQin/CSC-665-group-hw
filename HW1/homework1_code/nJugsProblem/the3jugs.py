...
class NJugsProblem(SearchProblem):
    ...
    def succ(self, state, action):
        ...
        if kind == "fill":
            if state[i] == self.capacities[i]:
                raise ValueError("Cannot fill a full jug.")

            new_state[i] = self.capacities[i]

        elif kind == "empty":
            if state[i] == 0:
                raise ValueError("Cannot empty an empty jug.")
            ...
            if state[i] == 0:
                raise ValueError("Cannot pour from an empty jug.")
            if state[j] == self.capacities[j]:
                raise ValueError("Cannor pour into a full jug.")

            amount = min(
                state[i],
                self.capacities[j] - state[j]
            )
